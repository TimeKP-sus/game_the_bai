...
public partial class DoiThu : CharacterBody2D
{
    ...
    public override void _Ready()
    {
        ...
        // GetNode<AnimatedSprite2D>("icon").GlobalPosition = new Vector2(1489, 255);
    ...
    public void TanCong()
    {
        ...
        tancong.SetParallel().TweenProperty(GetNode<AnimatedSprite2D>("icon"), "global_position", GetNode<AnimatedSprite2D>("icon").GlobalPosition - new Vector2(850, 0), 0.8f);
        tancong.Play();
        GetNode<AnimatedSprite2D>("icon").Play("attack");
        tancong.Finished += () =>
        {
            GetNode<AnimatedSprite2D>("icon").GlobalPosition = new Vector2(1489, 255);
            GetNode<NguoiChoi>("../nguoi_choi").BinhThuong();
            BinhThuong();
        };
